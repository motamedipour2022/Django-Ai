...
def Home(request):
    #API = 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
    lang_list = ['c', 'cfscript', 'clike', 'cpp', 'csharp', 'css', 'dart', 'django', 'go', 'html', 'java', 'javascript', 'markup', 'markup-templating', 'matlab', 'mongodb', 'objectivec', 'perl', 'php', 'powershell', 'python', 'r', 'regex', 'ruby', 'rust', 'sass', 'scala', 'sql', 'swift', 'yaml']
    ...
    if request.method == "POST":
        ...
        if lang == "Select Programming Language":
            ...
            return render(request, 'home.html', {'lang_list':lang_list, 'code':code, 'lang':lang})
                ...
                # OPEN AI key
                openai.api_key = "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"
...
def login_user(request):
        if request.method == "POST":
            ...
            if user is not None:
                login(request, user)
                messages.success(request, "You have been logged! wow")
                return redirect('home')
            else:
                messages.success(request, "Error check details")
                return redirect('home')
        else:
...
def register_user(request):
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                user =authenticate(request, username=username, password=password )
                login(request, user)
                messages.success(request, "You Have Register wellcome")
                return redirect('home')
